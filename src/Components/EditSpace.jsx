...
const { spaceId } = useParams();
  ...
  const fetchAvailableUsers = async () => {
    try {
      const res = await fetch("http://localhost:8000/api/spaces/available-users", {
        credentials: "include",
      });
  ...
  const fetchSpaceDetails = async () => {
    try {
      const res = await fetch(`http://localhost:8000/api/spaces/getSpace/${spaceId}`, {
        credentials: "include",
      });
      const result = await res.json();
      ...
      if (res.ok) {
        setData({
          ...
          members: result.data.members.map((m) => m._id), // get only IDs
        });
      } else {
  ...
  const handleSubmit = async () => {
    try {
      const res = await fetch(`http://localhost:8000/api/spaces/editSpace/${spaceId}`, {
        method: "PUT",
        credentials: "include",
        headers: {
      ...
      if (res.ok) {
        alert("Space updated!");
        navigate("/home/spaceDashboard"); // redirect after creation
      } else {
